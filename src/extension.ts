...
function commentPictureFormat(pictures: commentPictureJSON[]) {
	...
	for (let picture of pictures)
		if (commentImageWidth > 0)
			content += `<img class="comment" src="${atob(picture.src)}"/>`
		else
			content += '#图片已屏蔽#'
	return content + '</div>'
}
